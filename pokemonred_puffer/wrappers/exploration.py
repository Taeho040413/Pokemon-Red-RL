...
def _update_nested_seen_coords(
    seen_coords: dict[int, dict[tuple[int, int, int], float]], factor: float, floor: float = 0.15
):
    for tileset, coords in seen_coords.items():
        seen_coords[tileset] = {k: max(floor, v * factor) for k, v in coords.items()}


def _set_nested_seen_coords_fixed(
    seen_coords: dict[int, dict[tuple[int, int, int], float]], fixed: float
):
    for tileset, coords in seen_coords.items():
        seen_coords[tileset] = {k: fixed for k, v in coords.items() if v > 0}
...
class DecayWrapper(gym.Wrapper):
    ...
    def step_forget_explore(self):
        _update_nested_seen_coords(
            self.env.unwrapped.seen_coords, self.step_forgetting_factor["coords"]
        )
...
class MaxLengthWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if coord := self.cache.put((player_x, player_y, map_n)):
            x, y, n = coord
            for coords in self.env.unwrapped.seen_coords.values():
                coords.pop((x, y, n), None)
...
class OnResetLowerToFixedValueWrapper(gym.Wrapper):
    ...
    def step(self, action):
        if self.env.unwrapped.step_count >= self.env.unwrapped.get_max_steps():
            _set_nested_seen_coords_fixed(self.env.unwrapped.seen_coords, self.fixed_value["coords"])
